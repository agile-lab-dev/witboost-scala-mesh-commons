...
trait S3GatewayTestSupport {
  ...
  def getV(key: Int): ObjectVersion =
    ObjectVersion
      .builder()
      .eTag(s"eTag$key")
      .size(0)
      .storageClass("STANDARD")
      .key(s"key$key")
      .versionId(s"version$key")
      .isLatest(true)
      .lastModified(instant)
      .owner(Owner.builder().displayName("owner").id("ownerId").build())
      .build()

  def getD(key: Int): DeleteMarkerEntry =
    DeleteMarkerEntry
      .builder()
      .key(s"key$key")
      .versionId(s"version$key")
