...
import it.agilelab.provisioning.aws.s3.gateway.S3GatewayError.ObjectExistsErr
...
class DefaultS3GatewayTest extends AnyFunSuite with MockFactory with S3GatewayTestSupport {
  ...
  Seq(
    ("bucket", "my/file/key.txt"),
    ("bucket2", "my/file/key2.txt")
  ) foreach { case (bucket: String, key: String) =>
    test(s"objectExists with $bucket and $key return Right(true)") {
      (s3Client
        .headObject(_: HeadObjectRequest))
        .expects(HeadObjectRequest.builder().bucket(bucket).key(key).build())
        .once()
        .returns(HeadObjectResponse.builder().contentType("x").build())
    ...
    test(s"objectExists with $bucket and $key return Right(false)") {
      (s3Client
        ...
        .expects(HeadObjectRequest.builder().bucket(bucket).key(key).build())
        .once()
        .returns(HeadObjectResponse.builder().contentType(null).build())
  ...
  Seq(
    ...
    test(s"createFolder return Right() with $bucket, $folder") {
      (s3Client
        ...
        .expects(where { (r: PutObjectRequest, b: RequestBody) =>
          r.bucket() == expectedBucket &&
          r.key() == expectedFolder &&
          r.metadata().equals(Map("Content-Type" -> "application/x-directory").asJava) &&
          b.optionalContentLength().equals(Optional.of(0L))
        })
        .once()
  ...
  Seq(
    ("bucket", "my/file/key.txt", "Hello World"),
    ("bucket2", "my/file/key2.txt", "Hello World 2!")
  ) foreach { case (bucket: String, key: String, content: String) =>
    test(s"createFile return Right() with $bucket $key $content") {
      (s3Client
        ...
        .expects(where { (r: PutObjectRequest, rb: RequestBody) =>
          r.bucket() == bucket &&
          r.key() == key &&
          new String(rb.contentStreamProvider().newStream().readAllBytes()).contains(content)
        })
    ...
    test(s"createFile return Left(PutObjectErr) with $bucket $key $content") {
      (s3Client
        .putObject(_: PutObjectRequest, _: RequestBody))
        .expects(*, *)
        .throws(SdkClientException.create("xyz"))
    ...
    test(s"getObjectContentAsByteArray return Right(Array[Byte]) with $bucket, $key") {
      inSequence(
        (s3Client
          ...
          .expects(where { (r: GetObjectRequest) =>
            r.bucket() == bucket &&
            r.key() == key
          })
          .once()
  ...
  Seq(
    ("bucket", None),
    ("bucket", Some("prefix"))
  ) foreach { case (bucket: String, prefix: Option[String]) =>
    test(s"listObjects return Right(Iterator) with $bucket, $prefix") {
      inSequence(
        (s3Client
          .listObjectsV2Paginator(_: ListObjectsV2Request))
          .expects(where { (r: ListObjectsV2Request) =>
            r.bucket() == bucket &&
            r.prefix() == prefix.getOrElse("")
          })
          .once()
          .returns(
        ...
        (s3Client
          ...
          .expects(where { (r: ListObjectsV2Request) =>
            r.bucket() == bucket &&
            r.prefix() == prefix.getOrElse("") &&
            r.continuationToken() == "token1"
          })
          .once()
          .returns(
            ListObjectsV2Response
              .builder()
              .contents(getO(4), getO(5))
              .build()
          )
      )
  ...
  Seq(
    ...
    test(s"listVersions return Right(Iterator) with $bucket, $prefix") {
      inSequence(
        (s3Client
          .listObjectVersionsPaginator(_: ListObjectVersionsRequest))
          .expects(where { (r: ListObjectVersionsRequest) =>
            r.bucket() == bucket &&
            r.prefix() == prefix.getOrElse("")
          })
          .once()
          .returns(
            new ListObjectVersionsIterable(
              s3Client,
              ListObjectVersionsRequest
                .builder()
                .bucket(bucket)
        ...
        (s3Client
          .listObjectVersions(_: ListObjectVersionsRequest))
          .expects(where { (r: ListObjectVersionsRequest) =>
            r.bucket() == bucket &&
            r.prefix() == prefix.getOrElse("")
          })
      ...
      inSequence(
        ...
        (s3Client
          ...
          .returns(
            ListObjectVersionsResponse
              .builder()
              .deleteMarkers(getD(1), getD(2), getD(3))
              .build()
          )
      )
      //TODO assert should be more compact like in listObjects test cases
